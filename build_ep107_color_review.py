#!/usr/bin/env python3
# Genera ep107_color_review.html clonando estructura de ep106_color_review.html
# Episodio 107 — "Highway Heroes!" (Von Steele roba un tow truck con un auto valioso;
#   Sean y el escuadron deben rescatar el vehiculo fuera de control y capturar a Von Steele).
#   Villano: Von Steele.
# Color final: desde la REFINERIA (endpoint /finals). Mapeo panel->imagen 1:1
#   (46 finales: 01,02,04..47; el breakdown tiene P03 -title card- sin color).
...
TPL = open('ep106_color_review.html', encoding='utf-8').read()
...
# ---- 2. paneles EP107 desde trackerData ----
def unescape(s):
...
for line in BK.splitlines():
    mid = re.search(r"id:'ep107-p(\d+[a-z]?)'", line)
    if not mid: continue
...
url = 'https://refineria.onrender.com/api/projects/convoy/episodes/ep107/finals'
...
ENV = {'smalleys-garage':"SMALLEY'S GARAGE",'desert-highway':'DESERT HIGHWAY','city':'CITY','':''}
def scene_label(t):
...
# ---- 6. lineup: vehiculos hero del EP107 (segun GUION Highway Heroes V4) ----
# Reveal P04 (hero vehicles): Ted=Blaze Blaster III, Sean=Rover 6x6, Aimee=Sprinter Ambulance,
#   Crosby=F-650 Rollback, Charmaine=Rescue Helicopter.
# En la persecucion (P18/P19) el equipo cambia a rescue vehicles: Ted=Triumph Scrambler,
#   Crosby=Blaze Blaster III (su F-650 fue robado por Von Steele). Von Steele: Dodge Durango,
#   luego roba el F-650 y termina en el 2020 Nissan Nismo (el auto valioso transportado).
def vline(name, sku): return f'<div class="char-vehicle">{name} · #{sku}</div>'
LINEUP_VEH = {
 'Sean':      [('Land Rover 6x6','JKH28')],
 'Ted':       [('Triumph Scrambler','JKH31'), ('Blaze Blaster III','JHN10')],
 'Charmaine': [('Rescue Helicopter','JKH39')],
 'Aimee':     [('Mercedes Sprinter Ambulance','JKH35')],
 'Crosby':    [('F-650 Rollback','JKH49'), ('Blaze Blaster III','JHN10')],
}
# quitar TODOS los char-vehicle (EP106) e insertar los del EP107 tras cada char-role
...
# ---- vestuario EP107 (highway): recorta 5 pins CUADRADOS de la hoja combinada ----
# Mismo orden L->R que la hoja del EP106: Charmaine, Crosby, Sean, Aimee, Ted.
try:
    from PIL import Image
    import os
    os.makedirs('wardrobe/107', exist_ok=True)
    sheet = Image.open('wardrobe/107/CONVOY-uniforms-HIGHWAYb.jpg').convert('RGB')
    W, H = sheet.size
    bg = sheet.getpixel((8, 8))
    Hc = int(H * 0.86)  # recorta el texto "CONVOY EP07 HIGHWAY" de abajo
    ...
    for (a, b), nm in zip(runs, ORDER):
        ...
        cv.resize((1080, 1080), Image.LANCZOS).save('wardrobe/107/%s_EP107.png' % nm)
    print('[ok] pins de wardrobe/107 generados:', ', '.join(ORDER))
except ImportError:
    print('[warn] PIL no disponible: no se regeneraron los pins de wardrobe/107')

WARDROBE = {
    '106/SEAN_EP106.png':      '107/SEAN_EP107.png',
    '106/TED_EP106.png':       '107/TED_EP107.png',
    '106/CHARMAINE_EP106.png': '107/CHARMAINE_EP107.png',
    '106/AIMEE_EP106.png':     '107/AIMEE_EP107.png',
    '106/CROSBY_EP106.png':    '107/CROSBY_EP107.png',
}
...
reps = [
    ('CVY26_001_106','CVY26_001_107'),
    ('Episode 106 — Volcano Treasure!','Episode 107 — Highway Heroes!'),
    ('106 — Volcano Treasure!','107 — Highway Heroes!'),
    ('Episode 106 · Cast · Volcano Treasure','Episode 107 · Cast · Highway Heroes'),
    ('CHARACTER LINEUP PAGE — EP106 (Volcano Treasure)','CHARACTER LINEUP PAGE — EP107 (Highway Heroes)'),
    ('Episode 106','Episode 107'),
    ('Color Review · Volcano Treasure!','Color Review · Highway Heroes!'),
    ('Volcano Treasure! · 44 Panels','Highway Heroes! · %d Panels' % len(nums)),
    ('Volcano Treasure!','Highway Heroes!'),
    ('Volcano Treasure','Highway Heroes'),
    ('>44<', '>%d<' % len(nums)),
...
open('ep107_color_review.html','w',encoding='utf-8').write(out)
...
print('wardrobe: pins EP107 desde wardrobe/107/CONVOY-uniforms-HIGHWAYb.jpg')
print('\n== autos por panel ==')
for n in nums:
