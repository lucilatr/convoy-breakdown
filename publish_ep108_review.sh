#!/bin/zsh
# Publica ep108_color_review.html en R2 para que lo vea cualquiera.
# Sube las imagenes locales, reescribe las rutas a URLs publicas y sube el HTML.
# Uso:  ./publish_ep108_review.sh
set -e
cd "$(dirname "$0")"

WORKER="https://convoy-upload.valentin-e9a.workers.dev/upload"
SRC="ep108_color_review.html"
HTML_KEY="convoy/ep108_review/CVY26_001_108_Color_Review.html"
MAP="/tmp/ep108_asset_map.txt"
...
open('ep108_color_review_PUBLISHED.html','w',encoding='utf-8').write(h)
import re
left = re.findall(r'src="\./[^"]+"', h)
print('  rutas locales restantes:', left or 'ninguna')
PY

url=$(curl -s -X POST -F "file=@ep108_color_review_PUBLISHED.html;type=text/html; charset=utf-8" -F "key=${HTML_KEY}" "$WORKER" | sed -n 's/.*"url":"\([^"]*\)".*/\1/p')
echo
echo "✅ PUBLICADO:"
echo "$url"
